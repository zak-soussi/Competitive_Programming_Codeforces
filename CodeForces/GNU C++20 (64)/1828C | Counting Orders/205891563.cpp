...
int main()
{
    ...
    while (t--){
        ...
        vector<int>minims;
        for (int i = 0; i < n; ++i) {
            auto ind=a.end()- upper_bound(all(a),b[i]);
            minim=min(minim,(int)ind);
            minims.push_back((int)ind);
        }
        for (int i = 0; i < minims.size(); ++i) {
            if(minims[i]==minim)
            {
                minims.erase(minims.begin()+i);
                break;
            }
        }
        sort(all(minims));
        ...
        for (int i = 0; i < minims.size(); ++i) {
            count=((count%mod)*1LL*((minims[i]-(i+1))%mod))%mod;
            if(count<=0)
                count=0;
        }
        cout<<count<<nl;
