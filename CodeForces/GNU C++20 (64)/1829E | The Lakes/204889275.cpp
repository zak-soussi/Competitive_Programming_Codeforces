    ...
    while (t--){
        ...
        for (int i = 0; i < lakecells.size(); ++i) {
            if(visited[lakecells[i].first*m + lakecells[i].second])
                continue;
            int count=0;
            work(lakecells[i].first, lakecells[i].second, &count, n, m, grid);
