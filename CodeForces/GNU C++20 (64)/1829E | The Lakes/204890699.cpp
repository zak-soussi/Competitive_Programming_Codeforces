...
void work(int i,int j,int *occ, int n, int m, vector<vector<int>>& grid, bool * visited)
{
    (*occ)+=grid[i][j];
    visited[i*m+j] = 1;
//    cout<< "i j" << i << ' ' << j << nl;
     if((i+1<n)&&!visited[(i+1)*m+j]&&grid[i+1][j]>0)
         work(i+1,j,occ, n, m, grid,visited);
    if((i-1>=0)&&!visited[(i-1)*m+j]&&grid[i-1][j]>0)
        work(i-1,j,occ, n, m, grid,visited);
    if((j+1<m)&&!visited[i*m+j+1]&&grid[i][j+1])
        work(i,j+1,occ, n, m, grid,visited);
    if((j-1>=0)&&!visited[i*m+j-1]&&grid[i][j-1])
        work(i,j-1,occ, n, m, grid,visited);
}
...
int main()
{
    ...
    while (t--){
        ...
        fill(visited,visited+1000010,false);
        for (int i = 0; i < n; ++i) {
                grid[i].resize(m);
                for (int j = 0; j < m; ++j) {
                int x;
        ...
        for (int i = 0; i < lakecells.size(); ++i) {
            if(visited[lakecells[i].first*m + lakecells[i].second])
                continue;
            int count=0;
            work(lakecells[i].first, lakecells[i].second, &count, n, m, grid,visited);
