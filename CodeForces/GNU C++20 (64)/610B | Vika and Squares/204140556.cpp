...
//    cin>>t;
    while (t--){
        ...
        for (int i = 0; i < n; ++i) {
            if(a[i]== minim)
                indmin.emplace_back(i);
        }
        if(indmin.size()==1)
            cout<<n*1LL*minim+n-1<<nl;
        else
        {
            int minimminim=n-indmin.back()+indmin.front();
            for (int i = 0; i < indmin.size()-1; ++i) {
                minimminim=max(minimminim,indmin[i+1]-indmin[i]);
            }
            cout<<n*1LL*minim+minimminim-1<<nl;
        }
    }
    return 0;
}