...
//    cin>>t;
    while (t--){
        ...
        vector<vector<ll>> queries(m+1);
        for (int i = 1; i <= m; ++i) {
            queries[i].resize(3);
            cin>>queries[i][0]>>queries[i][1]>>queries[i][2];
        }
        vector<ll>nbr_mul(m+4,0);
        while (k--)
        {
            ll x,y;
            cin>>x>>y;
        ...
        vector<ll>nbr_mulXX(m+4,0);
        partial_sum(all(nbr_mul),nbr_mulXX.begin());
        vector<ll>nbr_add(n+4,0);
        for (int i = 1; i <= m; ++i) {
            nbr_add[queries[i][0]]+=nbr_mulXX[i]*queries[i][2];
            nbr_add[queries[i][1]+1]-=nbr_mulXX[i]*queries[i][2];
        }
        vector<ll>nbr_addXX(n+4,0);
        partial_sum(all(nbr_add),nbr_addXX.begin());
        for (int i = 1; i <=n ; ++i) {
            cout<<a[i]+nbr_addXX[i]<<" ";
        }
        cout<<nl;
    }
    return 0;
