...
bool can(vector<int> & a , int i , int m)
{
    return a[m]>=(i-m+1);
}
...
{
    int l=1,r=i,m,ans=-1;
//    while(can(r,n)) r*=2;   // send r to its max: the use of can() will depend on the problem we can use can() as well as !can() both of them are possible, ken 3ndik l right wadh7a,7otha toul(r= valeur wadh7a),sinon nst3mlou l methode hedhi
    while (l<=r)
    {
        m=l+(r-l)/2;   //do the work
        can(a,i,m)?r=m-1,ans=m:l=m+1;  //we only change ans when can is true(ALWAYS)
    }
    return ans;
}
