...
int main()
{
    ...
    while (t--){
        ...
        if(both.empty()&&(skill2.empty()||skill1.empty()))
            cout<<-1<<nl;
        else
        {
            int valboth=infiniint,valskill1=infiniint,valskill2=infiniint;
            if(!both.empty())
                valboth=*both.begin();
            if(!skill1.empty())
                valskill1=*skill1.begin();
            if(!skill2.empty())
                valskill2=*skill2.begin();
            cout<<min(valboth,valskill1+valskill2)<<nl;
        }
    }
    return 0;
}