...
//    cin>>t;
    while (t--){
        ...
        ll def=0,rightside,leftside;
        for (int i = 0; i < n; ++i) {
            if(web[i].size()==1)
                continue;
            for (int j = 0; j < web[i].size()-1; ++j) {
                rightside=0,leftside=0;
                auto ind= upper_bound(web[((i-1)+n)%n].begin(),web[((i-1)+n)%n].end(),web[i][j+1]);
                rightside+=(ind-web[((i-1)+n)%n].begin());
                ind= upper_bound(web[((i-1)+n)%n].begin(),web[((i-1)+n)%n].end(),web[i][j]);
                rightside-=(ind-web[((i-1)+n)%n].begin());
                ind= upper_bound(web[(i+1)%n].begin(),web[(i+1)%n].end(),web[i][j+1]);
                leftside+=(ind-web[(i+1)%n].begin());
                ind= upper_bound(web[(i+1)%n].begin(),web[(i+1)%n].end(),web[i][j]);
                leftside-=(ind-web[(i+1)%n].begin());
