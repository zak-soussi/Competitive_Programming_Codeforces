...
//checks if a cell of coordinates (i,j) is valid in a grid of dimensions n,m
...
void countLeafNodes(vector<v32> & tree , int vertex , v32 & occ , v32 & children) {
    ...
    else
    {
        ...
        for (auto child: tree[vertex]) {
            if (!occ[child]) {
                if (children[child] != -1)
                    leafCount += children[child];
                else {
                    countLeafNodes(tree, child, occ, children);
                    leafCount += children[child];
                }
            }
        }
        children[vertex]=leafCount;
    }
}
...
//    ::freopen("...","r",stdin);  // use this with input files
    ...
    while (t--) {
        ...
        countLeafNodes(tree,1,occ,children);
        int q;
        cin>>q;
        int l,r;
        while (q--)
        {
            cin>>l>>r;
            cout<<children[l]*1LL*children[r]<<nl;
        }
    }
    return 0;
}