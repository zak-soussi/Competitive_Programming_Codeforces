...
//    cin>>t;
    while (t--){
        ...
        if(!ABC.empty())
            result=*ABC.begin();
        if(!onlyA.empty()&&!onlyB.empty()&&!onlyC.empty())
            result=min(result,*onlyA.begin()+*onlyB.begin()+*onlyC.begin());
        if(!AB.empty())
        {
            if(!CB.empty())
                result=min(result,*AB.begin()+*CB.begin());
            if(!AC.empty())
                result=min(result,*AB.begin()+*AC.begin());
            if(!onlyC.empty())
                result=min(result,*AB.begin()+*onlyC.begin());
        }
        if(!AC.empty())
        {
            if(!CB.empty())
                result=min(result,*AC.begin()+*CB.begin());
            if(!onlyB.empty())
                result=min(result,*AC.begin()+*onlyB.begin());
        }
        if(!CB.empty()&&!onlyA.empty())
            result=min(result,*CB.begin()+*onlyA.begin());
        result != infinill ? cout << result << nl : cout << -1 << nl;
    }
    return 0;
}